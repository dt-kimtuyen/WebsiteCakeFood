    ...
    <div v-else class="orders-list">
      <div v-for="order in paginatedOrders" :key="order.id" class="order-card">
        <div class="order-header">
          <div>
            <span class="order-number">Order Code: {{ order._id }}</span>
  ...
  loading.value = false;
    ...
    const cancelOrder = async (orderId) => {
      ...
      try {
        ...
        { status: 'cancelled' }, // hoặc status khác như 'shipped', 'delivered'
        {
          headers: {
    ...
    const reorder = async (orderId) => {
      ...
      try {
        const response = await axios.put(`http://localhost:3000/api/orders/${orderId}`, 
        { status: 'pending' }, // hoặc status khác như 'shipped', 'delivered'
        {
          headers: {
            Authorization: `Bearer ${token}`
          }
        ...
        console.log('Đặt lại đơn hàng:', response.data)
